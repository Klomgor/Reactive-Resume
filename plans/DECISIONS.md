...
The final selected directions for plans 02 and 07–11 are their scoped recovery, separate PostgreSQL, self-hosted root routing, local JSON/Git documentation, prospective owner-only retired-link notices (public 404 unchanged), and current-workflow documentation respectively. These supersede earlier broader backup-sync and redirect proposals. Plan 32 is one-time sorting, not persistent autosort; plan 23 defers widow/orphan UI. Each plan must disclose partial issue coverage in its PR.
