...
The final selected directions for plans 02 and 07–11 are their scoped recovery, separate PostgreSQL, self-hosted root routing, local JSON/Git documentation, and current-workflow documentation respectively. Plan 10's earlier prospective owner-only retired-link notice was superseded on 2026-09-06: the maintainer declined it because redirect lifecycle and error-handling overhead are disproportionate, so it must not be implemented. These decisions supersede earlier broader backup-sync and redirect proposals. Plan 32 is one-time sorting, not persistent autosort; plan 23 defers widow/orphan UI. Each plan must disclose partial issue coverage in its PR.
